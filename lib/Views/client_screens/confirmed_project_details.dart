...
class ConfirmedProjectDetails extends StatefulWidget {
  ...
  @override
  State<ConfirmedProjectDetails> createState() => _ConfirmedProjectDetailsState();
}

class _ConfirmedProjectDetailsState extends State<ConfirmedProjectDetails> {
  ...
  List<TalentModel> confirmedUserData = <TalentModel>[];
  ...
  Future getConfirmedData() async {
    confirmedUserData.clear();
    debugPrint('............/////////////////////////////// first Option');
    for (var confirmed in confirmedList) {
      FirebaseFirestore.instance
          .collection('models')
          .where('userId', isEqualTo: confirmed)
          .snapshots()
          .listen((event) {
        event.docs.forEach((element) {
          confirmedUserData.add(TalentModel.fromMap(element.data()));
        });
        setState(() {});
      });
    }
...
// You can can also directly ask the permission about its status.
  ...
  void initState() {
    ...
    widget.confirmed.forEach((element) {
      confirmedList = List<String>.from(element.map((x) => x.toString()));
      debugPrint(confirmedList.toString());
...
//                 }); },)
        ...
        body: SafeArea(
          child: confirmedList.isNotEmpty
              ?GridView.builder(
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                  crossAxisCount: 2,
                  crossAxisSpacing: 2.0,
                  mainAxisSpacing: 2.0  ),
              itemCount: confirmedUserData.length,
              itemBuilder: (BuildContext ctx, index) {
                return PendingModelItem(
                  userId:  confirmedUserData.elementAt(index).userId,
                  firstName: confirmedUserData.elementAt(index).firstName,
                  lastName: confirmedUserData.elementAt(index).lastName,
                  email: confirmedUserData.elementAt(index).email,
                  profilePicture: confirmedUserData.elementAt(index).profilePicture,
                  dateOfBirth: confirmedUserData.elementAt(index).dateOfBirth,
                  gender: confirmedUserData.elementAt(index).gender,
                  county: confirmedUserData.elementAt(index).county,
                  state: confirmedUserData.elementAt(index).state,
                  city: confirmedUserData.elementAt(index).city,
                  postalCode: confirmedUserData.elementAt(index).postalCode,
                  drivingLicense: confirmedUserData.elementAt(index).drivingLicense,
                  idCard: confirmedUserData.elementAt(index).idCard,
                  passport: confirmedUserData.elementAt(index).passport,
                  otherId: confirmedUserData.elementAt(index).otherId,
                  country: confirmedUserData.elementAt(index).country,
                  hairColor: confirmedUserData.elementAt(index).hairColor,
                  eyeColor: confirmedUserData.elementAt(index).eyeColor,
                  heightCm: confirmedUserData.elementAt(index).heightCm,
                  heightFeet: confirmedUserData.elementAt(index).heightFeet,
                  waistCm: confirmedUserData.elementAt(index).waistCm,
                  waistInches: confirmedUserData.elementAt(index).waistInches,
                  weightKg: confirmedUserData.elementAt(index).weightKg,
                  weightPound: confirmedUserData.elementAt(index).weightPound,
                  buildType: confirmedUserData.elementAt(index).buildType,
                  wearSizeCm: confirmedUserData.elementAt(index).wearSizeCm,
                  wearSizeInches: confirmedUserData.elementAt(index).wearSizeInches,
                  chestCm: confirmedUserData.elementAt(index).chestCm,
                  chestInches: confirmedUserData.elementAt(index).chestInches,
                  hipCm: confirmedUserData.elementAt(index).hipCm,
                  hipInches: confirmedUserData.elementAt(index).hipInches,
                  inseamCm: confirmedUserData.elementAt(index).inseamCm,
                  inseamInches: confirmedUserData.elementAt(index).inseamInches,
                  shoesCm: confirmedUserData.elementAt(index).shoesCm,
                  shoesInches: confirmedUserData.elementAt(index).shoesInches,
                  scarPicture: confirmedUserData.elementAt(index).scarPicture,
                  tattooPicture: confirmedUserData.elementAt(index).tattooPicture,
                  isDrivingLicense: confirmedUserData.elementAt(index).isDrivingLicense,
                  isIdCard: confirmedUserData.elementAt(index).isIdCard,
                  isPassport: confirmedUserData.elementAt(index).isPassport,
                  isOtherId: confirmedUserData.elementAt(index).isOtherId,
                  isScar: confirmedUserData.elementAt(index).isScar,
                  isTattoo: confirmedUserData.elementAt(index).isTattoo,
                  languages: confirmedUserData.elementAt(index).languages,
                  sports: confirmedUserData.elementAt(index).sports,
                  hobbies: confirmedUserData.elementAt(index).hobbies,
                  talent: confirmedUserData.elementAt(index).talent,
                  portfolioImageLink: confirmedUserData.elementAt(index).portfolioImageLink,
                  portfolioVideoLink: confirmedUserData.elementAt(index).portfolioVideoLink,
                  polaroidImageLink: confirmedUserData.elementAt(index).polaroidImageLink,
                  projectId: widget.projectId,
                  indexNumber: index,
                );
              })
