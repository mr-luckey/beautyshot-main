...
class _ChestScreenState extends State<ChestScreen> {
  ...
  var chestUnit = MeasurementUnit.cm.toString().split('.').last;
  double chestInches = 0.0;
  double chestCm = 0.0;
  String? massageText;
  ...
  onForwardPress()async{
    ...
    if(chestCm == 0.0){
      ...
      SharedPreferences prefs = await _prefs;
      prefs.setDouble('chestCm', chestCm);
      prefs.setDouble('chestInches', chestInches);
      debugPrint('chestCm $chestCm');
      debugPrint('chestInches $chestInches');
      Navigator.push(context, PageTransition(type: PageTransitionType.bottomToTop, child: const HipScreen(),duration: const Duration(milliseconds: 500)));
    }
  }
                          ...
                          children: const [
                            Material(
                              ...
                              child: Padding(
                                ...
                                child: Text("What is your chest size?",
                                  style: TextStyle(
                                    fontSize: 15.0,
                                    color: Colors.black,
                                  ),
                                  ...
                                  FlutterSwitch(
                                      ...
                                      onToggle: (val) {
                                        setState(() {
                                          status = val;
                                          if(!val){
                                            chestUnit = MeasurementUnit.cm.toString().split('.').last;
                                            chestController.text = chestCm.toStringAsFixed(2);
                                          }
                                          else{
                                            chestUnit = MeasurementUnit.inch.toString().split('.').last;
                                            chestController.text = chestInches.toStringAsFixed(2);
                                          }
                                        });
                                      }),
                                  const SizedBox(
                                      ...
                                      onTap: () {
                                       if(status == false) {
                                         Picker(
                                            ...
                                            hideHeader: false,
                                            title: const Center(
                                                child: Text("Chest Size")),
                                            onConfirm: (Picker picker, List value) {
                                              setState(() {
                                                chestController.text = value.toString().substring(1,value.toString().length-1);
                                                chestCm = double.parse(chestController.text) ;
                                                chestInches = chestCm / 2.54 ;
                                              });
                                         ...
                                         Picker(
                                             ...
                                             onConfirm: (Picker picker, List value) {
                                               setState(() {
                                                 chestController.text = value.toString().substring(1,value.toString().length-1);
                                                 chestInches = double.parse(chestController.text) ;
